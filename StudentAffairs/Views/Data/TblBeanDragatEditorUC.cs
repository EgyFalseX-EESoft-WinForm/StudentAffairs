...
{
    public partial class TblBeanDragatEditorUC : XtraUserControl
    {
        #region - Var -
        private static readonly log4net.ILog Logger = log4net.LogManager.GetLogger(typeof(TblBeanDragatEditorUC));
        Datasource.dsData.RuleDetailRow _elementRule = null;
        #endregion
        #region - Fun -
        public TblBeanDragatEditorUC(Datasource.dsData.RuleDetailRow RuleElement)
        {
            InitializeComponent();
        ...
        private void UOW_BeforeCommitTransaction(object sender, DevExpress.Xpo.SessionManipulationEventArgs e)
        {
            // Get NewId
            int id = 0;
            object obj = Classes.Managers.DataManager.GetNewId("TblBeanDragat", "BeanDragatId");
            if (obj == null)
            {
            ...
            foreach (DevExpress.Xpo.Metadata.XPDataTableObject item in Rows)
            {
                ...
                if (item.GetMemberValue("BeanDragatId") == null)
                {
                    item.SetMemberValue("BeanDragatId", id);
                    id++;
                }
            }
        }
        ...
        private void repositoryItemButtonEditDetails_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            if (row.GetMemberValue("BeanDragatId") != null)
            {
                tbldragatEditorFrm frm = new tbldragatEditorFrm(Convert.ToInt32(row.GetMemberValue("BeanDragatId")));
                frm.ShowDialog();
            }
        }
        #endregion

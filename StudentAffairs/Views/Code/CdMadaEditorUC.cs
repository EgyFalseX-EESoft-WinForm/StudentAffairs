...
{
    public partial class CdMadaEditorUC : XtraUserControl
    {
        #region - Var -
        private static readonly log4net.ILog Logger = log4net.LogManager.GetLogger(typeof(CdMadaEditorUC));
        Datasource.dsData.RuleDetailRow _elementRule = null;
        #endregion
        #region - Fun -
        public CdMadaEditorUC(Datasource.dsData.RuleDetailRow RuleElement)
        {
            InitializeComponent();
        ...
        private void UOW_BeforeCommitTransaction(object sender, DevExpress.Xpo.SessionManipulationEventArgs e)
        {
            // Get NewId
            int id = 0;
            object obj = Classes.Managers.DataManager.GetNewId("CdMada", "MadaId");
            if (obj == null)
            {
            ...
            {
                if (item.GetMemberValue("MadaId") == null)
                {
                    item.SetMemberValue("MadaId", id);
                    id++;
                }
            }
        }

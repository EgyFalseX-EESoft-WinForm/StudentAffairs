...
{
    public partial class cdStageEditorUC : XtraUserControl
    {
        #region - Var -
        private static readonly log4net.ILog Logger = log4net.LogManager.GetLogger(typeof(cdStageEditorUC));
        Datasource.dsData.RuleDetailRow _elementRule = null;
        #endregion
        #region - Fun -
        public cdStageEditorUC(Datasource.dsData.RuleDetailRow RuleElement)
        {
            InitializeComponent();
        ...
        private void UOW_BeforeCommitTransaction(object sender, DevExpress.Xpo.SessionManipulationEventArgs e)
        {
            // Get NewId
            int id = 0;
            object obj = Classes.Managers.DataManager.GetNewId("cdStage", "StageId");
            if (obj == null)
            {
            ...
            {
                if (item.GetMemberValue("StageId") == null)
                {
                    item.SetMemberValue("StageId", id);
                    id++;
                }
            }
        }

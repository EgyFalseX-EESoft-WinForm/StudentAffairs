...
{
    public partial class cdYearEditorUC : XtraUserControl
    {
        #region - Var -
        private static readonly log4net.ILog Logger = log4net.LogManager.GetLogger(typeof(cdYearEditorUC));
        Datasource.dsData.RuleDetailRow _elementRule = null;
        #endregion
        #region - Fun -
        public cdYearEditorUC(Datasource.dsData.RuleDetailRow RuleElement)
        {
            InitializeComponent();
        ...
        private void UOW_BeforeCommitTransaction(object sender, DevExpress.Xpo.SessionManipulationEventArgs e)
        {
            // Get NewId
            int id = 0;
            object obj = Classes.Managers.DataManager.GetNewId("cdYear", "YearId");
            if (obj == null)
            {
            ...
            {
                if (item.GetMemberValue("YearId") == null)
                {
                    item.SetMemberValue("YearId", id); item.SetMemberValue("chide", false);
                    id++;
                }
            }
        }

...
{
    public partial class cdDeptEditorUC : XtraUserControl
    {
        #region - Var -
        private static readonly log4net.ILog Logger = log4net.LogManager.GetLogger(typeof(cdDeptEditorUC));
        Datasource.dsData.RuleDetailRow _elementRule = null;
        #endregion
        #region - Fun -
        public cdDeptEditorUC(Datasource.dsData.RuleDetailRow RuleElement)
        {
            InitializeComponent();
        ...
        private void UOW_BeforeCommitTransaction(object sender, DevExpress.Xpo.SessionManipulationEventArgs e)
        {
            // Get NewId
            int id = 0;
            object obj = Classes.Managers.DataManager.GetNewId("cdDept", "DeptId");
            if (obj == null)
            {
            ...
            {
                if (item.GetMemberValue("DeptId") == null)
                {
                    item.SetMemberValue("DeptId", id);
                    id++;
                }
            }
        }

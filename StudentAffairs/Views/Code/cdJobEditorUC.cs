...
{
    public partial class cdJobEditorUC : XtraUserControl
    {
        #region - Var -
        private static readonly log4net.ILog Logger = log4net.LogManager.GetLogger(typeof(cdJobEditorUC));
        Datasource.dsData.RuleDetailRow _elementRule = null;
        #endregion
        #region - Fun -
        public cdJobEditorUC(Datasource.dsData.RuleDetailRow RuleElement)
        {
            InitializeComponent();
        ...
        private void UOW_BeforeCommitTransaction(object sender, DevExpress.Xpo.SessionManipulationEventArgs e)
        {
            // Get NewId
            int id = 0;
            object obj = Classes.Managers.DataManager.GetNewId("cdJob", "JobId");
            if (obj == null)
            {
            ...
            {
                if (item.GetMemberValue("JobId") == null)
                {
                    item.SetMemberValue("JobId", id);
                    id++;
                }
            }
        }
